...
$tahun_list_query = mysqli_query($koneksi, "SELECT DISTINCT tahun FROM spm_berjalan ORDER BY tahun DESC");
$tahun_list = [];
...
$cek_data = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) as total FROM spm_berjalan WHERE tahun = $tahun"));
if($cek_data['total'] == 0 && !empty($tahun_list)) {
    // Jika tahun yang dipilih kosong, gunakan tahun yang pertama punya data
    $tahun = $tahun_list[0];
}

$q = mysqli_query($koneksi, "SELECT * FROM spm_berjalan WHERE tahun = $tahun ORDER BY id ASC");

$total = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT SUM(jumlah_belanja) AS total FROM spm_berjalan WHERE tahun = $tahun"));
                ...
                <tbody>
                  <?php while ($d = mysqli_fetch_assoc($q)): ?>
                    <tr>
                      ...
                      <td class="right"><?= number_format($d['jumlah_belanja'], 2, ',', '.') ?></td>
                      <td>
