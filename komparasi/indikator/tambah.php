...
if(isset($_POST['simpan'])) {
    ...
    $capaian = (float)($_POST['capaian'] ?? 0);
    
    // Validasi
    if (!$tahun_anggaran) {
        $error = "Tahun Anggaran harus diisi!";
    } else if ($capaian < 0 || $capaian > 100) {
        $error = "Capaian harus antara 0-100!";
    } else {
        // Cek duplikat
        $cek = mysqli_query($koneksi, "SELECT id FROM komparasi_indikator_kinerja WHERE tahun_anggaran = $tahun_anggaran");
        
        if (mysqli_num_rows($cek) > 0) {
            $error = "Data tahun $tahun_anggaran sudah ada!";
        } else {
            $sql = "INSERT INTO komparasi_indikator_kinerja (tahun_anggaran, capaian) VALUES ($tahun_anggaran, $capaian)";
...
<head>
    ...
    <title>Tambah Data Indikator Kinerja</title>
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form method="POST" action="">
                        ...
                        <div class="form-group mb-3">
                            <label for="capaian">Capaian (%) <span class="text-danger">*</span></label>
                            <input type="number" id="capaian" name="capaian" class="form-control" 
                                   step="0.01" min="0" max="100" value="<?= isset($_POST['capaian']) ? (float)$_POST['capaian'] : 0 ?>" 
